...
- **Codex fast mode:** per-agent `other_config.fast_mode` → `AgentData.ParseFastMode()` → `LoopConfig.FastMode` → `Options[OptFastMode]` → `service_tier:"priority"` set in `codex_build.go` (gpt-5.4/5.5/5.6/6 families only; other models silently skip — mirrored in the UI's `fast-mode-section.tsx`). Wire value is "priority" — "fast" is only the config-facing name and the ChatGPT backend rejects it. ChatGPT OAuth model catalog is hand-curated in `internal/http/provider_models_catalog.go`, with reasoning levels in `reasoning_capability.go`. Upstream does have `GET /backend-api/codex/models?client_version=` (the Codex app's source of truth: slugs, reasoning levels, service tiers, retirement dates) — not yet called by GoClaw
